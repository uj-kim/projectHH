...
const CartPage: React.FC = () => {
  ...
  >({
    ...
    queryFn: ({ pageParam = 0 }) =>
      ...
      user
        ? getCartItems(user.id, pageParam as number, PAGE_SIZE)
        : Promise.resolve([]),
  ...
  const totalPrice = allCartItems.reduce(
    (total, item) => total + item.product.price * item.order_quantity,
    0
  );
    ...
    <div className="p-6">
      ...
      {!user ? (
        ...
        <>
          ...
          {isLoading && <p>로딩 중...</p>}
          {isError && (
            <p className="text-red-500 mb-4">오류 발생: {error.message}</p>
          )}
