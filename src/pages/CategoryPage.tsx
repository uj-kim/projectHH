...
const CategoryPage: React.FC = () => {
    ...
    const {
        ...
        error: categoryError,
    } = useQuery<{ category_id: string; category_name: string; category_image_url: string | null }, Error>({
    ...
    if (!category_id) {
        return <p className="text-center text-red-500">유효하지 않은 카테고리입니다.</p>;
    }
    ...
    if (isCategoryError) {
        return <p className="text-center text-red-500">에러: {categoryError.message}</p>;
    }
    if (isProductsError) {
        return <p className="text-center text-red-500">에러: {productsError.message}</p>;
    }
