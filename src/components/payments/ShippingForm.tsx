...
const ShippingForm: React.FC = () => {
    ...
    const { shippingAddress, setShippingAddress, isUsingDefault, setIsUsingDefault } = usePaymentStore();
    ...
    const mutation = useMutation({
        mutationFn: (newAddress: string) => updateDefaultAddress(userId, newAddress),
        onSuccess: () => {
            toast.success('기본 배송지로 등록되었습니다.');
            setIsUsingDefault(true);
            queryClient.invalidateQueries({ queryKey: ['defaultShippingAddress', userId] });
        },
        onError: (error: Error) => {
    ...
    const handleBlur = () => {
        if (!isUsingDefault && shippingAddress.trim() !== '' && shippingAddress !== defaultAddress) {
            const askSave = window.confirm('입력한 배송지를 기본 배송지로 등록하시겠습니까?');
            if (askSave) {
    ...
    if (isAuthError) return <div className="text-red-500">배송지 로딩 오류: {authError.message}</div>;
